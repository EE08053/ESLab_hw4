...
const static char DEVICE_NAME[] = "B08901053";
...
private:
    ...
    void on_init_complete(BLE::InitializationCompleteCallbackContext *params) {
        ...
        _led_service = new LEDService(_ble, _led1); //new a LEDService and an IDService
        _id_service = new IDService(_ble, (char *)"B08901053", event_queue);
        _event_queue.call(Callback<void()>(_id_service, &IDService::sendIDValue)); //send our student ID
        _button.fall(Callback<void()>(this, &BatteryDemo::button_pressed));
        _button.rise(Callback<void()>(this, &BatteryDemo::button_released));

        // call _led_service -> updateLEDState to read data from client to change led1 status
        _event_queue.call_every(500, Callback<void()>(_led_service, &LEDService::updateLEDState));
        start_advertising();
    }
    ...
    void start_advertising() {
        ...
        ble::AdvertisingParameters adv_parameters(
            ble::advertising_type_t::CONNECTABLE_UNDIRECTED, // advertising type
            ble::adv_interval_t(ble::millisecond_t(1000))  // min interval // max interval // PDU used
        );
        ...
        ble_error_t error = _ble.gap().setAdvertisingParameters(
            ble::LEGACY_ADVERTISING_HANDLE, // advertising handle
            adv_parameters // advertising parameters
        );
